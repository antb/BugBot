...
class config:
    ...
    def conf_user(self):
        ...
        self.ident = raw_input('What is your bots username? (default: BugBot){0}'.format(self.n))
        self.chkresponse(self.ident,'self.conf_user()')
        if self.ident is '': self.ident = 'BugBot'
        self.conf_rname()

    def conf_rname(self):
        self.realname = raw_input('{0}What\'s your bots \'Real\' Name? (default: BugBot) {1}'.format(self.b,self.n))
        self.chkresponse(self.realname,'self.conf_rname()')
        if self.realname is '': self.realname = 'BugBot'
        self.conf_server()

    def conf_server(self):
        print '{0}Now I need to know how to connect to the server of your choice.'.format(self.b)
        self.server = raw_input('Server address? {0}'.format(self.n))
        self.chkresponse(self.server,'self.conf_server()')
        if self.server is '': self.conf_server()
        self.conf_port()

    def conf_port(self):
        self.port = raw_input('{0}Port number? (default: 6667) {1}'.format(self.b,self.n))
    ...
    def conf_nick(self):
        print '{0}Please enter two nicknames for your bot to use. (defaults are random nicks)'.format(self.b)
        self.nick1 = raw_input('Nick 1 -> {0}'.format(self.n))
        self.chkresponse(self.nick1,'self.conf_nick()')
        self.nick2 = raw_input('{0}Nick 2 -> {1}'.format(self.b,self.n))
    ...
    def conf_nickserv(self):
        nsid = raw_input('{0}Please enter your bots Nickserv username -> {1}'.format(self.b,self.n))
        self.chkresponse(nsid,'self.conf_nickserv()')        
        nspw = raw_input('{0}Please enter your bots Nickserv password (will be shown) -> {1}'.format(self.b,self.n))
    ...
    def conf_channels(self):
        print '{1}Specify some channels you\'d like {0} to join when its identified. Seperate each channel with a space.'.format(self.nick1,self.b)
        self.channels = raw_input('-> {0}'.format(self.n))
    ...
    def conf_prefix(self):
        self.prefixes = raw_input('{0}Enter the prefixes for your bot to respond to -> {1}'.format(self.b,self.n))
    ...
    def doMenuChoice(self):
        num = raw_input('[1-8]-> {0}'.format(self.n))
        try: num = int(num)
    ...
    def generate(self):
        ...
        print '{0}Config file generated.'.format(self.b)
        ...
        print 'When this command is recieved it will write your hostmask to the owner.{0}'.format(self.n)
        exit()
